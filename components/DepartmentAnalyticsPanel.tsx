        ...
        <div className="h-full bg-slate-100 border-l border-slate-300 flex flex-col shadow-xl">
            <div className="p-4 border-b border-slate-200 bg-white shadow-sm">
                ...
                {hasSelection ? (
                    ...
                    <p className="text-sm text-slate-400 mt-1">Select dates in value stream header to view capacity.</p>
                )}
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 custom-scrollbar bg-slate-50">
                {hasSelection ? (
                            ...
                            return (
                                <div key={dept} className="group bg-white p-3 rounded-lg border border-slate-200 shadow-sm transition-all hover:shadow-md">
                                    ...
                                    {splitByProductType && totalPoints > 0 && (
                                        <div className="mt-3 pt-2 border-t border-slate-100 flex gap-3 text-xs font-bold">
                                            <span className="text-blue-600 bg-blue-50 px-2 py-0.5 rounded">F:{Math.round(splitData.FAB)}</span>
                                            <span className="text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded">D:{Math.round(splitData.DOORS)}</span>
                                            <span className="text-violet-600 bg-violet-50 px-2 py-0.5 rounded">H:{Math.round(splitData.HARMONIC)}</span>
                                        </div>
                                    )}
                                </div>
                            );
                    ...
                    <div className="flex flex-col items-center justify-center h-40 text-slate-400 space-y-2">
                        ...
                        <p className="text-sm text-center">Click date headers<br />to inspect capacity</p>
                    </div>
                )}
            </div>
            ...
            {hasSelection && (
                <div className="p-4 border-t border-slate-200 bg-white shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                    <div className="flex justify-between items-center mb-1">
                        <span className="text-sm font-semibold text-slate-500">Total Selection Load</span>
